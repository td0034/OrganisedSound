#!/usr/bin/env python3
"""
Organised Sound Results Generator (Qual + Quant)
------------------------------------------------

Inputs (auto-detected unless overridden):
  - data.json
  - sections_plus_transcriptions.json

Outputs:
  - outputs/summary_numbers.json
  - outputs/log.txt
  - outputs/tables/*.csv (merged blocks, descriptives, tests, qualitative extracts)
  - outputs/figures/core/*.eps + .png
  - outputs/figures/additional/*.eps + .png

Design goals:
  - Robust to slightly different JSON schemas (dict/list nesting, answers as dict or list).
  - Merge pre + post records per participant + condition (A/B/C).
  - Create publication-ready EPS figures for Organised Sound.

Notes:
  - This script does not touch MIDI logs or audio feature extraction.
...
TS_KEYS = [
    "created_at", "createdAt", "timestamp", "saved_at", "savedAt",
...
EXTRA_KEYS = {
    ...
    "param_other", "preset_id", "one_change", "reflection", "biggest_mismatch",
    "session_notes_transcription", "perceptual_comments", "order", "meta_notes",
    "session_type", "age_range", "color_deficiency", "light_sensitivity",
    "musical_experience", "theory_familiarity", "tonnetz_familiarity",
    "generative_experience", "participant_id", "dyad_done",
}
            ...
            if ("id" in obj or "qid" in obj or "name" in obj) and ("value" in obj or "answer" in obj or "response" in obj):
                ...
                for k, v in obj.items():
                    if isinstance(k, str) and (
                        k.startswith("A_") or k.startswith("B_") or k in PREF_KEYS
                        or k in PARAM_INFLUENCE_KEYS or k in EXTRA_KEYS
                    ):
                        out[k.strip()] = v
        elif isinstance(obj, list):
            for it in obj:
...
def pick_latest_record(g: pd.DataFrame) -> pd.Series:
...
def merge_pre_post(df_records: pd.DataFrame, log_lines: List[str]) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    # Keep only A_ items in pre, B_ in post, plus key qualitative fields
    pre_keep = ["participant_id", "condition", "timestamp", "section_raw"] + A_ITEMS + [
        "aim", "strategy", "preset_id",
    ]
    post_keep = ["participant_id", "condition", "timestamp", "section_raw"] + B_ITEMS + [
        "expectation_vs_outcome", "interference_notes", "param_influence", "param_other", "preset_id",
    ]
    ...
    df_pre_last = df_pre.groupby(["participant_id", "condition"], dropna=False).apply(pick_latest_record).reset_index(drop=True)
    df_post_last = df_post.groupby(["participant_id", "condition"], dropna=False).apply(pick_latest_record).reset_index(drop=True)
...
def latest_by_section(df: pd.DataFrame, keyword: str) -> pd.DataFrame:
    if "section_raw" not in df.columns or "participant_id" not in df.columns:
        return pd.DataFrame()
    out = df[df["section_raw"].str.contains(keyword, case=False, na=False)].copy()
    if out.empty:
        return out
    out = out[out["participant_id"].notna() & (out["participant_id"].astype(str).str.strip() != "")]
    if out.empty:
        return out
    out["_ord"] = np.arange(len(out))
    return out.groupby("participant_id", dropna=False).apply(pick_latest_record).reset_index(drop=True)
...
def main() -> None:
    ...
    for name in ["data.json", "sections_plus_transcriptions.json", "sections.json", "sections_edited.json"]:
        candidate = os.path.join(base_dir, name)
        if os.path.exists(candidate):
            default_sections = candidate
            break

    ap.add_argument("--sections", default=default_sections,
                    help="Path to data.json or sections_plus_transcriptions.json")
    ap.add_argument("--addendum", required=False, help="Optional addendum JSON")
    ap.add_argument("--out", default=os.path.join(base_dir, "outputs"),
                    help="Output directory for tables/summaries")
    ap.add_argument("--fig", default=os.path.join(base_dir, "outputs", "figures"),
                    help="Root output directory for EPS+PNG figures (core/ additional)")
    args = ap.parse_args()

    if not args.sections:
        ap.error("No input JSON found. Provide --sections PATH.")
    ...
    # Qualitative exports
    # -------------------------
    df_background = latest_by_section(df_records, "background")
    df_meta = latest_by_section(df_records, "meta")
    df_dyad = latest_by_section(df_records, "dyad")

    if not df_background.empty:
        cols = [
            "participant_id", "age_range", "color_deficiency", "light_sensitivity",
            "musical_experience", "theory_familiarity", "tonnetz_familiarity",
            "generative_experience", "perceptual_comments",
        ]
        df_background[select_existing_columns(df_background, cols)].to_csv(
            os.path.join(args.out, "tables", "background_responses.csv"), index=False
        )

    if not df_meta.empty:
        cols = ["participant_id", "order", "session_type", "meta_notes"]
        df_meta[select_existing_columns(df_meta, cols)].to_csv(
            os.path.join(args.out, "tables", "meta_sessions.csv"), index=False
        )

    if not df_dyad.empty:
        cols = ["participant_id", "dyad_done"]
        df_dyad[select_existing_columns(df_dyad, cols)].to_csv(
            os.path.join(args.out, "tables", "dyad_gate.csv"), index=False
        )

    # Latest end-of-session record per participant (used for rankings + qualitative)
    df_end_last = pd.DataFrame()
    if not df_end.empty:
        df_end = df_end.copy()
        df_end["_ord"] = np.arange(len(df_end))
        df_end_last = df_end.groupby("participant_id", dropna=False).apply(pick_latest_record).reset_index(drop=True)

        end_cols = [
            "participant_id", "rank_A", "rank_B", "rank_C", "most_intermedial",
            "one_change", "reflection", "biggest_mismatch", "session_notes_transcription",
        ]
        df_end_last[select_existing_columns(df_end_last, end_cols)].to_csv(
            os.path.join(args.out, "tables", "end_qualitative.csv"), index=False
        )

    if not df_blocks.empty:
        block_cols = [
            "participant_id", "condition",
            "aim_pre", "strategy_pre",
            "expectation_vs_outcome_post", "interference_notes_post",
            "param_influence_post", "param_other_post",
            "preset_id_pre", "preset_id_post",
        ]
        df_blocks[select_existing_columns(df_blocks, block_cols)].to_csv(
            os.path.join(args.out, "tables", "block_qualitative.csv"), index=False
        )
